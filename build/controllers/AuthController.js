    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 1:
                user = _b.sent();
                if (!user) {
                    return [2 /*return*/, response.status(404).json({ error: 'Usuario não encontrado' })];
                }
                // verifica se a senha esta correta
                if (!user.checkIfPasswordMatch(password)) {
                    return [2 /*return*/, response.status(401).json({ error: 'Senha incorreta' })];
                }
                return [2 /*return*/, response.status(202).json({
                        user: {
                            id: user.id,
                            email: user.email,
                            name: user.name,
                        },
                        token: jwt.sign({ id: user.id }, authConfig_1.default.secret, {
                            subject: user.id,
            ...
            case 2:
                ...
                return [2 /*return*/, response
                        .status(500)
                        .json({ error: "Alguma coisa deu errado " + error_1 })];
